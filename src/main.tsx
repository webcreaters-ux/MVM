...
function App() {
  ...
  function updateProvider(id: string, patch: Partial<ModelProvider>) {
    ...
    if (patch.enabled) setSelectedProviderId(id);
  }

  function resetChat() {
    clearChat(); setMessages([]); setError('');
  }
  ...
  return <main className={dark ? 'app dark' : 'app'}>
    <aside className="sidebar">
      ...
      <div className="side-bottom"><button className="nav" onClick={() => setDark(!dark)}>◐ {dark ? 'Light mode' : 'Dark mode'}</button><span className="version">MVM 0.3 • OPEN SOURCE</span></div>
    </aside>
    <section className="content">
      ...
      {active === 'chat' ? <section className="workspace">
        <div className="workspace-head"><div><span className="pill">AI HUB • {provider.name}</span><h2>Talk to your model.</h2><p>Local-first routing with Ollama and OpenAI-compatible endpoints.</p></div><div className="head-actions"><select value={selectedProviderId} onChange={e => setSelectedProviderId(e.target.value)}>{providers.filter(p => p.enabled).map(p => <option value={p.id} key={p.id}>{p.name}</option>)}</select><button className="ghost" onClick={resetChat}>New chat</button><button className="ghost" onClick={() => setActive('settings')}>Configure →</button></div></div>
      ...
      <footer>Designed for PC + mobile • Provider-neutral architecture • Local chat history • No API keys shipped in the client</footer>
    </section>
  </main>;
}
