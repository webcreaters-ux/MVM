...
function effectiveModel(request: RouterRequest) {
  return request.provider.model?.trim() || request.model;
}
...
async function callOllama(request: RouterRequest): Promise<RouterResponse> {
  ...
  if (!response.ok) throw new Error(`Ollama returned HTTP ${response.status}. Check that Ollama is running and install the selected model (${model}).`);
  const data = await response.json() as { message?: { content?: string } };
  return { content: data.message?.content ?? 'The local model returned an empty response.', provider: request.provider.name, model };
}
...
async function callOpenAICompatible(request: RouterRequest): Promise<RouterResponse> {
  ...
  return { content: data.choices?.[0]?.message?.content ?? 'The provider returned an empty response.', provider: request.provider.name, model };
}
